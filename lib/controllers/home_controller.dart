...
class HomeController extends GetxController {
  ...
  void _loadNotifications() {
    ...
    if (currentUserId != null) {
      _notifications.bindStream(
        _firestoreService.getUserNotificationsStream(currentUserId),
      );
    }
  }
  ...
  void _sortSearchResults(String query) {
    filteredChats.sort((a, b) {
      ...
      // التحقق من القيم الفارغة لتجنب الأخطاء
      if (userA == null || userB == null) return 0;

      final exactMatchA =
          userA.displayName?.toLowerCase().startsWith(query.toLowerCase()) ??
          false;

      final exactMatchB =
          userB.displayName?.toLowerCase().startsWith(query.toLowerCase()) ??
          false;
  ...
  void searchUserByLastMessage(String lastMessage) {
    onSearchChanged(lastMessage);
  }
  ...
  List<String> getSearchSuggestions() {
    ...
    for (var chat in _allChats) {
      ...
      // التحقق من أن المستخدم واسمه ليسا بقيم فارغة
...
Future<void> deleteChat(ChatModel chat) async {
  try {
    ...
    if (result == true) {
      _isLoading.value = true; // بداية التحميل قبل الحذف
